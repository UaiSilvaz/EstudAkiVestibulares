...
export default function Masonry({
  ...
  const getInitialPosition = (item: { x: number; y: number; w: number; h: number }) => {
    ...
    let direction: "top" | "bottom" | "left" | "right" | "center";
  ...
  const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>, item: { id: string }) => {
    const selector = `[data-key="${item.id}"]`;
    if (scaleOnHover) {
  ...
  const handleMouseLeave = (e: React.MouseEvent<HTMLDivElement>, item: { id: string }) => {
    const selector = `[data-key="${item.id}"]`;
    if (scaleOnHover) {
  ...
  return (
    <div
      ref={containerRef}
      className="list"
      style={{ height: Math.max(...grid.map((i) => i.y + i.h), 600) }}
    >
      {grid.map((item) => (
        <div
          key={item.id}
        ...
        >
          ...
          >
            {item.badge && (
              <span className="absolute left-3 top-3 z-10 inline-flex items-center gap-1 rounded-full bg-white/90 px-3 py-1 text-[10px] font-black uppercase tracking-wider text-[#06245C] shadow">
                {item.badge}
              </span>
            )}
            {(item.title || item.subtitle) && (
              <div className="absolute bottom-0 left-0 right-0 z-10 p-4 text-white">
                {item.title && (
                  <p className="text-sm font-black leading-tight drop-shadow-md sm:text-base">
                    {item.title}
                  </p>
                )}
                {item.subtitle && (
