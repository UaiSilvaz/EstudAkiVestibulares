  ...
  if (data.accessDenied) {
    return (
      <div className="mx-auto max-w-3xl space-y-5">
        <FastLink href={`/cursos/${data.course.slug}`} className="inline-flex h-10 items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 text-sm font-black text-[var(--brand)]">
          <ArrowLeft className="h-4 w-4" />
          Voltar para trilha
        </FastLink>
        <section className="rounded-[32px] border border-amber-100 bg-[var(--surface)] p-8 text-center shadow-[0_24px_64px_-44px_rgba(15,23,42,0.42)]">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-[28px] bg-amber-50 text-amber-600">
            <Lock className="h-9 w-9" />
          </div>
          <h1 className="mt-5 font-display text-3xl font-black text-[var(--text)]">
            {data.deniedReason === "purchase" ? "Aula exclusiva deste curso" : "Atividade ainda bloqueada"}
          </h1>
          <p className="mx-auto mt-3 max-w-xl text-sm font-semibold leading-6 text-[var(--text-secondary)]">
          ...
          <div className="mt-6 flex justify-center gap-3">
            {data.deniedReason === "purchase" && <button type="button" onClick={() => setCheckoutOpen(true)} className="h-12 rounded-full bg-[var(--brand-button)] px-5 text-sm font-black text-white">Comprar curso</button>}
            <FastLink href={`/cursos/${data.course.slug}`} className="inline-flex h-12 items-center rounded-full border border-[var(--border)] px-5 text-sm font-black text-[var(--text)]">Ver trilha</FastLink>
          </div>
        </section>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
      <main className="min-w-0 space-y-5">
        ...
        <section className="rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[var(--brand)]">{data.course.title}</p>
              <h1 className="mt-1 font-display text-3xl font-black text-[var(--text)]">{data.lesson.title}</h1>
              <p className="mt-2 text-sm font-semibold text-[var(--text-secondary)]">{data.course.teacherName} - {formatCourseDuration(data.lesson.durationSeconds)}</p>
            </div>
        ...
        <section className="overflow-hidden rounded-[28px] border border-[var(--border)] bg-[var(--surface)] shadow-sm">
          <div className="flex gap-1 overflow-x-auto border-b border-[var(--border)] p-2">
            {[
              ...
              <button key={id} type="button" onClick={() => setTab(id as Tab)} className={cn("h-10 whitespace-nowrap rounded-full px-4 text-xs font-black transition", tab === id ? "bg-[var(--brand-button)] text-white" : "text-[var(--text-secondary)] hover:bg-[var(--surface-secondary)]")}>
                {label}
              </button>
            ))}
          </div>
      ...
      {completionOpen && (
        <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[30px] border border-white/80 bg-[var(--surface)] p-6 text-center shadow-2xl">
            <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-[28px] bg-emerald-50 text-emerald-600">
              <CheckCircle2 className="h-10 w-10" />
            </div>
            <h2 className="mt-4 font-display text-3xl font-black text-[var(--text)]">Muito bem!</h2>
            <p className="mt-2 text-sm font-bold text-[var(--text-secondary)]">+{data.lesson.xpReward} XP. Proxima atividade liberada.</p>
            <div className="mt-5 flex justify-center gap-3">
              {data.nextNode ? <FastLink href={data.nextNode.href} className="inline-flex h-12 items-center gap-2 rounded-full bg-[var(--brand-button)] px-5 text-sm font-black text-white">Continuar <ArrowRight className="h-4 w-4" /></FastLink> : <FastLink href={`/cursos/${data.course.slug}`} className="inline-flex h-12 items-center rounded-full bg-[var(--brand-button)] px-5 text-sm font-black text-white">Voltar para trilha</FastLink>}
              <button type="button" onClick={() => setCompletionOpen(false)} className="h-12 rounded-full border border-[var(--border)] px-5 text-sm font-black text-[var(--text)]">Fechar</button>
            </div>
          </div>
        </div>
      )}
...
function ActionButton({ active, onClick, label, icon }: { active: boolean; onClick: () => void; label: string; icon: React.ReactNode }) {
  return <button type="button" onClick={onClick} className={cn("inline-flex h-11 items-center gap-2 rounded-full border px-4 text-sm font-black transition", active ? "border-[var(--border)] bg-[var(--brand-soft)] text-[var(--brand)]" : "border-[var(--border)] bg-[var(--surface)] text-[var(--text-secondary)] hover:bg-[var(--surface-secondary)]")}>{icon}{label}</button>;
}
...
function AboutTab({ data }: { data: LessonPageDTO }) {
  return (
    <div className="grid gap-5 lg:grid-cols-[1fr_280px]">
      <div>
        <h2 className="font-display text-xl font-black text-[var(--text)]">Resumo da aula</h2>
        <p className="mt-2 text-sm font-semibold leading-6 text-[var(--text-secondary)]">{data.lesson.description}</p>
        <div className="mt-5">
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--brand)]">Voce aprendera</p>
          <ul className="mt-3 space-y-2">
            {data.lesson.outcomes.map((item) => <li key={item} className="flex gap-2 text-sm font-bold text-[var(--text)]"><CheckCircle2 className="mt-0.5 h-4 w-4 text-emerald-600" />{item}</li>)}
          </ul>
        </div>
      </div>
...
function NotesBox({ lessonId, initialContent, currentTime }: { lessonId: string; initialContent: string; currentTime: number }) {
  ...
  return (
    <div className="rounded-[22px] border border-[var(--border)] bg-[var(--surface-secondary)] p-4">
      <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--text-secondary)]">Minhas anotacoes</p>
      <textarea value={content} onChange={(event) => {
        setSaved(false);
        setContent(event.target.value);
      }} placeholder="Escreva suas anotacoes..." className="mt-3 min-h-36 w-full resize-none rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 text-sm font-semibold outline-none focus:border-[var(--border)]" />
...
function CommentsTab({ lessonId, kind, currentTime, endpoint }: { lessonId: string; kind: "QUESTION" | "DISCUSSION"; currentTime: number; endpoint: "questions" | "comments" }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[22px] border border-[var(--border)] bg-[var(--surface-secondary)] p-3">
        <div className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.14em] text-[var(--brand)]">
          <MessageCircle className="h-4 w-4" />
          Timestamp atual {formatTimestamp(currentTime)}
        </div>
        <textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder={kind === "QUESTION" ? "Qual sua duvida?" : "Comente sobre esta aula..."} className="mt-3 min-h-24 w-full resize-none rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 text-sm font-semibold outline-none focus:border-[var(--border)]" />
        <button type="button" onClick={() => void send()} className="mt-3 inline-flex h-10 items-center gap-2 rounded-full bg-[var(--brand-button)] px-4 text-sm font-black text-white"><Send className="h-4 w-4" />Enviar</button>
      </div>
      {loading ? <p className="text-sm font-bold text-[var(--text-secondary)]">Carregando...</p> : comments.length === 0 ? <p className="rounded-2xl bg-[var(--surface-secondary)] p-4 text-sm font-bold text-[var(--text-secondary)]">{kind === "QUESTION" ? "Alguma parte ficou confusa? Envie sua duvida." : "Seja o primeiro a comentar nesta aula."}</p> : comments.map((comment) => <CommentCard key={comment.id} comment={comment} />)}
    </div>
  );
}
  ...
  return (
    <article className="rounded-[22px] border border-[var(--border)] bg-[var(--surface)] p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="font-black text-[var(--text)]">{comment.user.name} {comment.user.role === "TEACHER" && <span className="rounded-full bg-[var(--brand-soft)] px-2 py-0.5 text-[10px] font-black uppercase text-[var(--brand)]">Professor</span>}</p>
        ...
        {comment.positionSeconds !== null && (
          <button
            ...
            className="rounded-full bg-[var(--brand-soft)] px-2.5 py-1 text-xs font-black text-[var(--brand)]"
          >
            {formatTimestamp(comment.positionSeconds)}
          </button>
        )}
      </div>
      <p className="mt-3 text-sm font-semibold leading-6 text-[var(--text)]">{comment.body}</p>
      ...
      {comment.replies.length > 0 && <div className="mt-3 space-y-2 border-l border-[var(--border)] pl-3">{comment.replies.map((reply) => <CommentCard key={reply.id} comment={reply} />)}</div>}
    </article>
  );
}

function MaterialsTab({ resources }: { resources: Array<{ title: string; type: string; href: string }> }) {
  if (resources.length === 0) return <p className="rounded-2xl bg-[var(--surface-secondary)] p-4 text-sm font-bold text-[var(--text-secondary)]">Nenhum material complementar nesta aula.</p>;
  return <div className="grid gap-3 md:grid-cols-2">{resources.map((resource) => <a key={resource.title} href={resource.href} className="flex items-center gap-3 rounded-[22px] border border-[var(--border)] bg-[var(--surface-secondary)] p-4 transition hover:bg-[var(--brand-soft)]"><Download className="h-5 w-5 text-[var(--brand)]" /><span><span className="block font-black text-[var(--text)]">{resource.title}</span><span className="text-xs font-bold text-[var(--text-secondary)]">{resource.type}</span></span></a>)}</div>;
}
...
function QuestionsTab({ lessonId, questions, simulation }: { lessonId: string; questions: LessonQuestionDTO[]; simulation: LessonPageDTO["lesson"]["simulation"] }) {
  ...
  if (questions.length === 0) return <p className="rounded-2xl bg-[var(--surface-secondary)] p-4 text-sm font-bold text-[var(--text-secondary)]">As questoes desta aula aparecerao aqui quando forem cadastradas.</p>;
  ...
  return (
    <div className="space-y-4">
      ...
      {questions.map((question, index) => {
        ...
        return (
          <article key={question.id} className="rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-4">
            <p className="text-[10px] font-black uppercase tracking-[0.14em] text-[var(--brand)]">Questao {index + 1} - +{question.xpReward} XP</p>
            <h3 className="mt-2 text-base font-black leading-6 text-[var(--text)]">{question.statement}</h3>
            <div className="mt-4 grid gap-2">
              {question.alternatives.map((alternative) => (
                <button key={alternative.key} type="button" onClick={() => setAnswers((current) => ({ ...current, [question.questionId]: alternative.key }))} className={cn("flex min-h-11 items-center gap-3 rounded-2xl border px-3 text-left text-sm font-bold transition", answers[question.questionId] === alternative.key ? "border-[var(--brand)] bg-[var(--brand-soft)] text-blue-800" : "border-[var(--border)] bg-[var(--surface-secondary)] text-[var(--text)]")}>
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-[var(--surface)] text-xs font-black">{alternative.key}</span>
                  {alternative.text}
                </button>
              ))}
            </div>
            ...
            {result && <div className={cn("mt-3 rounded-2xl p-4 text-sm font-bold", result.correct ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700")}><p>{result.correct ? `Correto! +${result.xpAwarded} XP` : `Ainda nao. Alternativa correta: ${result.correctAlternative}`}</p><p className="mt-1 text-[var(--text-secondary)]">{result.explanation}</p></div>}
          </article>
        );
      })}
      ...
      {simResult && <p className="rounded-2xl bg-[var(--brand-soft)] p-4 text-sm font-black text-[var(--brand)]">{simResult}</p>}
    </div>
  );
}
  ...
  return (
    <aside className="space-y-3 xl:sticky xl:top-24">
      {data.modules.map((module) => (
        <section key={module.id} className="rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm">
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[var(--brand)]">{module.eyebrow}</p>
          <h2 className="mt-1 font-black text-[var(--text)]">{module.title}</h2>
          <div className="mt-3 space-y-2">
            {module.nodes.map((node) => (
              <FastLink key={node.id} href={node.href} className={cn("flex items-center gap-3 rounded-2xl p-2 text-sm font-bold", node.id === data.lesson.id ? "bg-[var(--brand-soft)] text-[var(--brand)]" : "text-[var(--text-secondary)] hover:bg-[var(--surface-secondary)]", node.state === "locked" && "pointer-events-none opacity-50")}>
